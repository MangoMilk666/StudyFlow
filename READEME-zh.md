...
生产环境变量参考。本地开发时可以先复制为 `.env`，按需填写。

项目支持可选的 `.env.local`，在运行 `dev-up.sh` 时用于覆盖 `.env` 中同名变量。
...
本地开发便捷脚本，接受 `--mock`（以 Mock 模式启动后端，无需 MongoDB）或 `--real`（以真实 MongoDB 连接启动后端）参数，启动 FastAPI（uvicorn reload）与 Vite 开发服务器。
...
| 文件 | 内容 |
|---|---|
| `docs/Project_Statement.md` | 产品说明与系统概览 |
| `docs/database_schema.md` | MongoDB 集合 Schema 定义 |
| `docs/design/` | 线框图与 HTML 原型 |
